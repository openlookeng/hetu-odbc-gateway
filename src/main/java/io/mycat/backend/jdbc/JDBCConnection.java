...
public class JDBCConnection implements BackendConnection {
	...
	protected ServerConnection frontConn;
	protected volatile Object attachement;
	...
	protected boolean setSchemaFail = false;
	...
	public void setFrontConn(ServerConnection frontConn) {
	...
	public void commit() {
		try {
			...
			if (this.respHandler != null) {
				...
				/* commit empty trans, response OK */
				frontConn.write(frontConn.writeToBuffer(OkPacket.OK, frontConn.allocate()));
			}
		} catch (SQLException e) {
	...
	protected void syncIsolation(int nativeIsolation)
    {
	...
	protected void executeSQL(RouteResultsetNode rrn, ServerConnection sc,
	...
	protected void executeddl(ServerConnection sc, String sql)
			throws SQLException {
		Statement stmt = null;
		try {
	...
	protected void ouputCallStatement(RouteResultsetNode rrn,ServerConnection sc, String sql)
			throws SQLException {
            ...
            if(procedure.isResultList())
            {
                ...
                for (ProcedureParameter paramter : paramters)
                {
                            ...
                            || ProcedureParameter.INOUT.equalsIgnoreCase(paramter.getParameterType()))  )
                    {
                        ...
                        if (this instanceof HetuConnection) {
	...
	protected void ouputResultSet(ServerConnection sc, String sql)
            throws SQLException {
        ResultSet rs = null;
        Statement stmt = null;
				...
				for (int i = 0; i < colunmCount; i++) {
					...
					if(MysqlDefs.isBianry((byte) fieldPks.get(i).type)) {
						...
						curRow.add(StringUtil.encode(val != null ? val.toPlainString() : null,
								sc.getCharset()));
					} else if(fieldPks.get(i).type == MysqlDefs.FIELD_TYPE_BIT) {
							//boolean map to bit, cant't use getString()
							curRow.add(StringUtil.encode(rs.getBoolean(j) ? "1":"0",
									sc.getCharset()));
					} else {
	...
	public void rollback() {
		try {
			...
			if (this.respHandler != null) {
				...
				/* rollback empty trans, response OK */
				frontConn.write(frontConn.writeToBuffer(OkPacket.OK, frontConn.allocate()));
			}
		} catch (SQLException e) {
			throw new RuntimeException(e);
