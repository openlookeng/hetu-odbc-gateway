...
public class PreparedStmtResponse {
    ...
    public static void response(PreparedStatement pstmt, FrontendConnection c, int[] parameterType) {
        byte packetId = 0;
        ...
        if (parametersNumber > 0) {
            for (int i = 0; i < parametersNumber; i++) {
                ...
                // add parameterType in field packet
                field.type = (parameterType[i] < 0) ? MysqlDefs.FIELD_TYPE_VAR_STRING : parameterType[i];
        ...
        List<FieldPacket> packets = pstmt.getOutFieldPks();

        if (columnsNumber > 0) {
            for (int i = 0; i < columnsNumber; i++) {
                FieldPacket field = packets.get(i);
                field.packetId = ++packetId;
                buffer = field.write(buffer, c,true);
            }
            EOFPacket eof = new EOFPacket();
